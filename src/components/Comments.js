...
function Comments({comment, user, image, timestamp, id}) {
    ...
    return (
        <div className={css `background-color: #F4F4F6; margin: 4px; padding: 10px; overflow-y: auto; `}>
          <div className={css `margin: 6px; display: flex; align-items: center; `}>
            <img className={css `height: 50px; width: 50px; object-fit: contadfsn; border-radius: 50px; `} src={image} alt=""/>
            ...
            <button onClick={edit}>Edit</button>
                ...
                <EditComment />
                 ...
                 <span className={css`float: right; color: #DDDDE3; cursor: pointer; `}onClick={deleteComment}>delete</span>
            </div>
±
        </div>
    )
